# Automatic parcel tracking from e-mail (IMAP → Correos)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Correos tracking code, and register it with `correos.track_parcel` — fully automatic, no extra custom component required.

Correos is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `correos.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ correos.track_parcel
                                     │
...
- This integration, with the `correos.track_parcel` action available (field `tracking_code`).
...
The regex matches Correos' two common code shapes:

```
(?<![0-9A-Za-z])(?:[A-Z]{2}[0-9]{9}ES|PQ[0-9A-Z]{8,})
```

That covers the **UPU S10** form (two letters + 9 digits + `ES`, e.g. `LX123456789ES`) and **`PQ…`** parcel codes. The integration itself accepts any code matching `^[A-Z0-9]{6,30}$`, but matching *that* broadly against e-mail text would hit every order number in the mail — so the example anchors on the known shapes instead. **If your mails carry a different prefix** (`PK`, `CP`, `DS`…), add it to the alternation; the AI fallback will surface unknown formats in the meantime.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@correosshipping.com","subject":"Your parcel is on its way",
       "text":"Tracking code: EXAMPLE123456","initial":true,"folder":"INBOX","username":"test"}'
```

Then `correos.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
