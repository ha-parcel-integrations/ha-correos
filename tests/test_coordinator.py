"""Tests for the Correos coordinator: fetching, caching and events.
...
from custom_components.correos.api import CorreosApiError
...
from custom_components.correos.coordinator import CorreosCoordinator
...
def _in_transit(code: str = ACTIVE_CODE) -> dict:
    """A sample whose latest event is 'Clasificado' (in transit)."""
    sample = active_sample(code)
    # Drop the 'En reparto' event so the current state is one hop earlier.
...
async def test_update_keeps_cached_payload_on_error(hass):
    ...
    client.async_get_parcel.side_effect = CorreosApiError("HTTP 500")
...
async def test_update_backfills_missing_tracking_number(hass):
    ...
    del sample["codEnvio"]  # no tracking number in the payload
    client = AsyncMock()
...
async def test_fires_delivery_time_changed_event(hass):
    ...
    Correos' consumer endpoint exposes no ETA, so ``planned_from`` is always
    ``None`` on a real parcel and this event never fires in practice. The
    branch is still shared suite code, so drive ``_fire_change_events``
    directly with a synthetic window shift rather than pretending Correos
    reports one.
    """
    entry = _entry_with([{CONF_TRACKING_CODE: ACTIVE_CODE}])
    entry.add_to_hass(hass)
    coordinator = CorreosCoordinator(hass, AsyncMock(), entry)
    ...
    coordinator._known_state = {ACTIVE_CODE: ParcelStatus.OUT_FOR_DELIVERY}
