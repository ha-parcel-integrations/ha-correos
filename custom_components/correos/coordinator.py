"""Coordinator for the Correos parcel tracker integration.
...
from .api import CorreosApiClient, CorreosApiError
from .const import (
...
class CorreosCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (CorreosApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("Correos fetch failed for %s: %s", code, result)
        ...
        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("Correos unreachable for all tracked parcels")
