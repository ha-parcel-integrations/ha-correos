"""Calendar platform for the Correos parcel tracker integration."""
...
async def async_setup_entry(
    ...
    """Set up the Correos deliveries calendar from a config entry."""
    async_add_entities([CorreosDeliveriesCalendar(entry.runtime_data.coordinator, entry)])


class CorreosDeliveriesCalendar(CoordinatorEntity[CorreosCoordinator], CalendarEntity):
    """A read-only calendar of expected Correos deliveries.
    ...
    def _events(self) -> list[CalendarEvent]:
        ...
        for parcel in self.coordinator.data or []:
            ...
            summary = sender or (f"Parcel {barcode}" if barcode else "Correos parcel")
            description_parts = [
