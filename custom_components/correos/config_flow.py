"""Config flow for the Correos parcel tracker integration."""
...
class CorreosConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    ) -> CorreosOptionsFlowHandler:
        """Return the options flow handler."""
        return CorreosOptionsFlowHandler()

    async def async_step_user(
        self, user_input: dict[str, Any] | None = None
    ) -> ConfigFlowResult:
        """Create the Correos hub — single instance, no input needed.
        ...
        the ``correos.track_parcel`` service or a dashboard button.
        ``single_config_entry`` in the manifest enforces one hub.

        Correos' ``localizador`` endpoint keys purely on the tracking code —
        no postcode — so there is nothing more to collect here.
        """
