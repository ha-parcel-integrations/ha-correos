"""Sensor platform for the Correos parcel tracker integration."""
...
from .coordinator import CorreosCoordinator
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: CorreosConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Correos sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        CorreosIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
        CorreosAwaitingPickupSensor(coordinator, entry),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            CorreosParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(CorreosNextDeliverySensor(coordinator, entry))
    entities.append(CorreosDeliveredParcelsSensor(coordinator, entry))
    entities.append(CorreosLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class CorreosAwaitingPickupSensor(
    CoordinatorEntity[CorreosCoordinator], SensorEntity
):
    ...
    def __init__(self, coordinator: CorreosCoordinator, entry: ConfigEntry) -> None:
...
class CorreosIncomingParcelsSensor(
    CoordinatorEntity[CorreosCoordinator], SensorEntity
):
    ...
    def __init__(
        self,
        coordinator: CorreosCoordinator,
        entry: ConfigEntry,
        async_add_entities: AddEntitiesCallback,
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                CorreosParcelSensor(self.coordinator, self._entry, barcode)
...
class CorreosParcelSensor(CoordinatorEntity[CorreosCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Correos parcel."""
    ...
    def __init__(
        self, coordinator: CorreosCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class CorreosNextDeliverySensor(
    CoordinatorEntity[CorreosCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: CorreosCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class CorreosDeliveredParcelsSensor(
    CoordinatorEntity[CorreosCoordinator], SensorEntity
):
    """Recently delivered tracked Correos parcels."""
...
class CorreosLastUpdateSensor(
    CoordinatorEntity[CorreosCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Correos was last polled successfully."""
