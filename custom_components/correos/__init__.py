"""Correos parcel tracker custom component for Home Assistant."""
...
from .api import CorreosApiClient
from .const import PLATFORMS
from .coordinator import CorreosCoordinator, _refresh_interval
...
@dataclass
class CorreosData:
    """Runtime data attached to the Correos config entry."""

    client: CorreosApiClient
    coordinator: CorreosCoordinator


type CorreosConfigEntry = ConfigEntry[CorreosData]


async def async_setup_entry(hass: HomeAssistant, entry: CorreosConfigEntry) -> bool:
    """Set up Correos from a config entry."""
    # No auth: Correos tracking is public, so the HA-managed session is fine.
    client = CorreosApiClient(async_get_clientsession(hass))
    ...
    entry.runtime_data = CorreosData(client=client, coordinator=coordinator)
...
async def async_unload_entry(hass: HomeAssistant, entry: CorreosConfigEntry) -> bool:
    """Unload the Correos config entry."""
